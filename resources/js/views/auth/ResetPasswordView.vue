<template>
  <v-card class="pa-5">
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn text color="primary" to="/auth/login" :disabled="!isLoaded"
        >{{ $t("base.button.cancel") }}
      </v-btn>
      <v-btn color="primary" :disabled="!isLoaded" @click="sendResetPassword"
        >{{ $t("base.button.reset") }}
      </v-btn>
    </v-card-actions>
  </v-card>
  ...
  methods: {
    sendResetPassword() {
      try {
        ...
        validationArray[this.$t("form.register.field.email")] =
          this.resetPasswordForm.email;
        validationArray[this.$t("form.resetPasswordForm.field.password")] =
          this.resetPasswordForm.password;
        validationArray[this.$t("form.resetPasswordForm.field.rePassword")] =
