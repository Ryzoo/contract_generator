<template>
  <v-card class="pa-5">
      ...
      <v-form v-if="isLoaded">
        ...
        <v-checkbox v-model="registerForm.rodoAccept">
          <template v-slot:label>
            ...
            <a target="_blank" href="http://google.pl" @click.stop> Rodo </a>
          </template>
        </v-checkbox>
    ...
    <v-card-actions>
      <v-spacer />
      <v-btn text color="primary" :disabled="!isLoaded" to="/auth/login"
        >{{ $t("base.button.login") }}
      </v-btn>
      <v-btn color="primary" :disabled="!isLoaded" @click="sendRegisterForm"
  ...
  methods: {
    sendRegisterForm() {
      try {
        ...
        validationArray[this.$t("base.field.firstName")] =
          this.registerForm.firstName;
        validationArray[this.$t("base.field.lastName")] =
          this.registerForm.lastName;
        validationArray[this.$t("base.field.email")] = this.registerForm.email;
        validationArray[this.$t("base.field.password")] =
          this.registerForm.password;
        validationArray[this.$t("base.field.rePassword")] =
          this.registerForm.rePassword;
        validationArray[this.$t("base.field.rodoAccept")] =
          this.registerForm.rodoAccept;
        validationArray[this.$t("base.field.regulationsAccept")] =
