                ...
                >
                    <template v-slot:item.action="{ item }">
                        <div class="table-icons">
                            ...
                            <v-icon @click="tryToRemoveContract(item.id)"
                                >fa-trash</v-icon
                            >
                        </div>
                    </template>
...
export default {
    ...
    methods: {
        tryToRemoveContract(id) {
            this.removeContractId = id;
            this.deleteDialog = true;
        },
        removeContract() {
            this.isLoaded = false;
            axios
                .delete(`/contract/${this.removeContractId}`)
                .then(response => {
                    this.contractItems = this.contractItems.filter(
                        e => e.id != this.removeContractId
                    );
