        ...
        <v-layout row wrap>
            <v-flex xs12>
                <v-card class="pa-5" v-if="isLoaded">
                        ...
                        <v-row>
                            <v-col cols="12" md="6">
                                ...
                                <v-text-field
                                    ...
                                    :label="
                                        $t(
                                            'form.contractAddForm.field.contract_name'
                                        )
                                    "
                                    @change="saveContractDataToStore"
                                    required
                                ...
                                <v-select
                                    ...
                                    :label="
                                        $t(
                                            'form.contractAddForm.field.contract_categories'
                                        )
                                    "
                                />
                                <vue-editor
                    ...
                    <v-card-actions>
                        <v-spacer />
                        <v-btn text @click="cancelAddContract" color="primary">
                            {{ $t('base.button.back') }}
                        </v-btn>
                        <v-btn outlined @click="saveAndExit" color="primary">
                            {{ $t('base.button.save_exit') }}
                        </v-btn>
                        <v-btn @click="saveAndBuild" color="primary">
    ...
    methods: {
        ...
        loadContract() {
            ...
            axios.get(`/contract/${this.contractId}`).then((response) => {
                this.$store
                    .dispatch('newContract_setUpdate', {
                        ...response.data,
