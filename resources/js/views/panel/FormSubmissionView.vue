          ...
          >
            ...
            <template v-slot:item.action="{ item }">
              <div class="table-icons">
                <v-btn color="primary" small v-if="(item.status === 3 || item.status === 2) && item.render_url"
                       type="application/octet-stream" :href="item.render_url" download>
                ...
                <span v-if="(item.status === 5) && item && item.action_details">
                  <v-btn color="primary" small v-if="item.action_details.payment_is_pending===true"
                         @click="goToPayment(item.action_details.payment_url)">
                  <v-icon small>fas fa-dollar-sign</v-icon>
                    {{ $t('base.button.pay') }}
                  </v-btn>
                  <v-btn color="primary" small v-if="item.action_details.payment_is_pending===false" disabled>
