<template>
    <v-card class="ma-auto" width="800px" v-if="isLoaded">
        <v-card-title>
            ...
            <v-spacer></v-spacer>
            <v-btn v-if="draftList.length" color="primary" @click="addVariable"
                >+ Add new attribute</v-btn
            >
        </v-card-title>
        ...
        <v-card-text>
                    ...
                    <v-list-item v-for="draft in draftList" :key="draft.id">
                        <v-list-item-content>
                            <v-list-item-title>
                                <v-chip
                                    v-if="
                                        draft.category && draft.category.length
                                    "
                                    class="mr-2"
                                    color="pink"
                                    small
                        ...
                        <v-list-item-icon>
                            <v-btn
                                ...
                                text
                                color="primary"
                                @click="() => editVariable(draft)"
                            >
                                Edit <v-icon small right>fa-edit</v-icon>
                            </v-btn>
                            <v-btn
                                small
                                text
                                color="error"
                                @click="() => deleteVariable(draft)"
                            >
                                Delete <v-icon small right>fa-trash</v-icon>
                            </v-btn>
                        </v-list-item-icon>
                    </v-list-item>
                </v-list>
            ...
            <v-alert type="info" prominent v-else>
                <v-row align="center">
                    <v-col class="grow"
                        >Any attributes in library. Create one!</v-col
                    >
                    <v-col class="shrink">
                        <v-btn color="white" outlined @click="addVariable"
                            >+ Add new attribute</v-btn
                        >
                    </v-col>
                </v-row>
        ...
        <v-dialog persistent v-model="showDeleteModal" max-width="290">
            <v-card>
                <v-card-title class="headline"
                    >Delete attribute from library</v-card-title
                >
    ...
    computed: {
        ...
        categoriesForDrafts() {
            return this.draftList
                .filter((x) => x.category && x.category.length)
                .map((x) => x.category)
        },
    },
    methods: {
        forceDeleteVariable() {
            this.isLoaded = false
            axios
                .delete('/library/variables/' + this.attributeToDelete)
                .then(() => {
                    this.showDeleteModal = false
                    this.$store.dispatch(
                        'library_attributes_deleteDraft',
        ...
        loadVariables() {
            ...
            axios
                ...
                .then((response) => {
                    this.$store.dispatch(
                        'library_attributes_setDraftList',
                        response.data
                    )
                })
                .finally(() => {
