<template>
  <v-card>
    ...
    <v-card-text class="my-2" v-if="draft && isLoaded">
      ...
      <div v-if="draft.content.length" class="text-center">
        <v-row>
          <v-col cols="12" md="4">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        rules: {
          ...
          description: [
            ...
            (v) =>
              v.length >= 3 || "Description must be grater than 3 characters",
          ],
      ...
      draft: null,
      isNew:
        this.editAttributeId === null || this.editAttributeId === undefined,
    };
  },
  watch: {
    editAttributeId() {
  ...
  computed: {
    defaultAttributes() {
      return this.draft.content
        ...
        .filter(
          (x) => parseInt(x.attributeType) !== AttributeTypeEnum.ATTRIBUTE_GROUP
        )
  ...
  methods: {
    ...
    saveDraft() {
      this.isLoaded = false;
      axios
        .put("/library/variables/" + this.editAttributeId, {
          ...this.draft,
    ...
    loadExistedDraftData() {
      this.draft = {
        ...this.$store.getters.library_attributes_getExistedDraft(
          this.editAttributeId
        ),
      };
      this.idCounter = Math.max(
        ...this.draft.content.map((x) => parseInt(x.id))
      );
    },
    loadNewDraftData() {
      this.draft = {
    ...
    removeAttribute() {
      this.draft.content = this.draft.content.filter(
        (x) => parseInt(x.id) !== parseInt(this.removedAttribute.id)
      );
