<template>
    <section>
        ...
        >
            ...
            <action-renderer
                v-if="
                    currentAction === availableActionsHook.BEFORE_FORM_END ||
                    currentAction === availableActionsHook.AFTER_FORM_END
                "
                v-model="currentAction"
                :contract="contract"
            />
                        ...
                        >
                            ...
                            <v-row v-if="actualStep === 1">
                                <v-col>
                                    <v-alert
                                        type="info"
                                        v-if="contract && contract.description"
                                    >
                                        <div v-html="contract.description" />
                                    </v-alert>
                                </v-col>
                            </v-row>
                            <form-renderer :formElements="step.content" />
...
export default {
    ...
    methods: {
        ...
        goToNextStep() {
            this.validateActual().then(() => {
                if (this.isCurrentStepValid()) {
                    this.actualStep =
                        this.actualStep === this.stepList.length
                            ? this.actualStep
                            : this.actualStep + 1
                }
            })
        ...
        isCurrentStepValid() {
            if (
                !this.stepList[this.actualStep - 1].content
                    .filter((x) => x.isActive)
                    .every((e) => e.isValid)
            ) {
                Notify.push(
                    this.$t('module.notify.completeAllElement'),
                    Notify.ERROR
                )
                return false
            }
        ...
        loadContractForm(additionalAttributes) {
            ...
            const additionalParam =
                this.getAttributesFromArrayWithObject(additionalAttributes)

            axios
                .get(
                    `/contract/form/${this.contractId}${
                ...
                .then((response) => {
                    ...
                    this.$store
                        .dispatch(
                            'formElements_set',
                            response.data.formElements
                        )
                        .then(() => {
                            this.currentAction =
                                this.availableActionsHook.FORM_RENDER
                ...
                .catch((err) => {
                    console.error(err)
                    this.currentAction =
                        this.availableActionsHook.BEFORE_FORM_RENDER
                    this.isLoading = false
                })
        },
        getAttributesFromArrayWithObject(additionalAttributes) {
            ...
            return defaultAttributeObject
                ? new URLSearchParams(defaultAttributeObject).toString()
                : ''
        },
        finishContractForm() {
            this.validateActual().then(() => {
                if (this.isCurrentStepValid()) {
                    this.currentAction =
                        this.availableActionsHook.BEFORE_FORM_END
                }
            })
        },
        init() {
        ...
        loadContractModules() {
            this.isLoading = true
            axios
                .get(`/contract/modules/${this.contractId}`)
                .then((response) => {
