...
class AttributeValidator {
  ...
  initialize(attribute) {
    if (!attribute) {
      throw new Error("Attribute validator initialized with null attribute");
    }
  ...
  validateRequired() {
    if (
      ...
      AttributeValidatorHelper.isEmpty(this.attribute)
    ) {
  ...
  validateLengthMin() {
    if (this.settings.lengthMin) {
      if (
        !AttributeValidatorHelper.isString(this.attribute) &&
        !AttributeValidatorHelper.isArray(this.attribute)
      ) {
      ...
      if (
        (AttributeValidatorHelper.isArray(this.attribute)
          ? this.value.length
          : String(this.value).length) < parseInt(this.settings.lengthMin)
      ) {
  ...
  validateLengthMax() {
    if (this.settings.lengthMax) {
      ...
      if (
        (AttributeValidatorHelper.isArray(this.attribute)
          ? this.value.length
          : String(this.value).length) > parseInt(this.settings.lengthMax)
      ) {
        const validationError = AttributeValidatorHelper.isArray(this.attribute)
...
class AttributeValidatorHelper {
  static isEmpty(attribute) {
    ...
    switch (parseInt(attribute.attributeType)) {
      ...
      case AttributeTypeEnum.BOOL_INPUT:
        return (
          ...
          value.input === "" ||
          (value.input.length === undefined && isNaN(value.input))
        );
    ...
    return (
      ...
      value === undefined ||
      value === "" ||
      (value.length === undefined && isNaN(value))
    );
  }
