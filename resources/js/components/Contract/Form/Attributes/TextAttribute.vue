<template>
    <v-col sm="12">
        <v-text-field
            ...
            :placeholder="
                String(attribute.defaultValue ? attribute.defaultValue : '')
            "
...
export default {
    ...
    methods: {
        ...
        isValid(newValue) {
            ...
            if (
                this.settings.lengthMin &&
                newValue.length < this.settings.lengthMin
            ) {
                this.validationError = this.$t("validation.min.string", {
            ...
            if (
                this.settings.lengthMax &&
                newValue.length > this.settings.lengthMax
            ) {
                this.validationError = this.$t("validation.max.string", {
                    attribute: this.attribute.name,
