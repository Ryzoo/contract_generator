<template>
  <v-card>
    <v-card-title>
      <v-row class="align-center">
        <v-col cols="12" md="auto">
          {{
            isNew
              ? $t("form.variableForm.new.title")
              : $t("form.variableForm.update.title")
          }}
        </v-col>
        <v-col>
          <v-select
    ...
    <v-card-text>
      <v-row v-if="attributeData.attributeType > -1">
          ...
          <v-expansion-panels>
            <v-expansion-panel>
              <v-expansion-panel-header
                >Additional configuration</v-expansion-panel-header
              >
              <v-expansion-panel-content>
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      dense
                      outlined
                      v-model="attributeData.placeholder"
                      :label="$t('form.variableForm.placeholder')"
                      outline
                    />
                  </v-col>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      :label="$t('form.variableForm.label')"
                      outline
                    />
                  </v-col>
                  <v-col cols="12">
                    <default-value
                      ...
                      v-model="attributeData.defaultValue"
                    ></default-value>
                  </v-col>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      dense
                      outlined
                      v-model="attributeData.additionalInformation"
                      :label="$t('form.variableForm.additionalInformation')"
                      outline
                    />
                  </v-col>
                  <v-col cols="12">
                    <v-text-field
                      ...
                      dense
                      outlined
                      v-model="attributeData.labelAfter"
                  ...
                  <v-col cols="12">
                    <v-checkbox
                      ...
                      dense
                      outlined
                      v-model="attributeData.toAnonymize"
              ...
              <v-expansion-panel-content>
                <v-row v-if="attribute && attribute.isInGroup">
                  <v-alert type="info" dense prominent width="100%">
                    This attribute is used in group. Can't be used as multiple
                    value.
                  </v-alert>
                </v-row>
                <v-row v-else>
                  <v-col cols="12">
                    <v-switch
                      ...
                      dense
                      outlined
                      v-model="attributeData.settings.isMultiUse"
                      :label="$t('form.variableForm.isMultiUse')"
                    ></v-switch>
                  </v-col>
                  <v-col
                    cols="12"
                    v-if="
                      attributeData.settings.isMultiUse &&
                      attributeData.attributeType !==
                        attributeType.ATTRIBUTE_GROUP
                    "
                  >
                    <v-switch
                      class="mt-0"
                      ...
                      dense
                      outlined
                      v-model="attributeData.settings.isInline"
                      :label="$t('form.variableForm.isInline')"
                    ></v-switch>
                  </v-col>
        ...
        <v-col
          cols="12"
          v-if="
            attributeData.attributeType === attributeType.ATTRIBUTE_GROUP &&
            attributeData.id !== undefined &&
        ...
        >
          <v-expansion-panels>
            <v-expansion-panel>
              ...
              <v-expansion-panel-content>
                <v-expansion-panels
                  v-for="attribute in attributeData.settings.attributes"
                  :key="attribute.id"
                >
                  <v-expansion-panel>
                    <v-expansion-panel-header>{{
    ...
    <v-card-actions>
      ...
      <v-btn color="primary" @click="saveVariable">{{
        isNew ? "Create" : "Save"
      }}</v-btn>
    </v-card-actions>
  </v-card>
...
export default {
  ...
  props: ["attribute", "noStore", "allAttributes"],
  components: {
    VariableSettings,
    DefaultValue,
    QueryBuilderForGroupsVar,
  ...
  computed: {
    attributes() {
      return this.noStore
        ? this.allAttributes || []
        : this.$store.getters.builder_allVariables;
    },
  },
  mounted() {
  ...
  methods: {
    ...
    getAttributeDefaultSettings() {
      ...
      axios
        ...
        .then((res) => {
          ...
          const attributeData = res.data.find(
            (x) => x.attributeType === this.attributeData.attributeType
          );
          this.setDefaultSettingsToNonSet(
            attributeData ? attributeData.settings : {}
          );
        })
        .finally(() => {
    ...
    isValid() {
      try {
        ...
        validationArray[this.$t("form.variableForm.name")] =
          this.attributeData.attributeName;
