<template>
    <section>
        <v-text-field
            v-if="
                acceptedType([type.TEXT, type.BOOL_INPUT, type.DATE, type.TIME])
            "
    ...
    watch: {
        attributeData(newValue) {
            ...
            if (
                this.isMultiSelect &&
                !Array.isArray(
                    this.currentValue && this.acceptedType(this.type.SELECT)
                )
            ) {
                this.currentValue = []
            }
        ...
        acceptedType(types) {
            if (Array.isArray(types)) {
                return types.some(
                    (x) =>
                        parseInt(x) ===
                        parseInt(this.attributeData.attributeType)
                )
            }
            return (
                parseInt(types) === parseInt(this.attributeData.attributeType)
            )
        },
        update() {
