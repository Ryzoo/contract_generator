    ...
    methods: {
        ...
        changeAttributes(newValue) {
            ...
            if (newValue.length < this.settingsData.attributes.length) {
                const removedAttributes = this.settingsData.attributes
                    .map((x) => x.id)
                    .filter((x) => !newValue.includes(x))
                if (!this.noStore) {
                    this.$store.dispatch(
                        'builder_attributes_clearConditionals',
                        removedAttributes
                    )
                } else {
            ...
            newValue.forEach((x) => {
                const attributes = this.attributesToAggregate.find(
                    (a) => a.id === x
                )
            ...
            this.settingsData.attributes = [...listOfAttributes]
