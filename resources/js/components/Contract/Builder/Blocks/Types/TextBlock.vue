            ...
            <div class="menubar-container">
                <div class="menubar">
                    ...
                    <button
                        ...
                        :class="{
                            'is-active': editor.isActive('paragraphList'),
                        }"
            ...
            handler(newValue) {
                if (this.editor && this.actualText !== newValue.content.text) {
                    this.editor.commands.setContent(
                        this.parseBlockContent(newValue)
                    )
                }
            },
        },
        ...
        variableUpdated() {
            return (
                (this.nestedVariables
                    ? this.nestedVariables
                    : this.$store.getters.builder_allVariables_defaultText) ||
                []
            ).map((x) => ({
                id: x.id,
    ...
    methods: {
                    ...
                    Mention.configure({
                            ...
                            render: () => {
                                ...
                                return {
                                    ...
                                    onExit(props) {
                                        ...
                                        this.actualText = `${element.prop(
                                            'innerHTML'
                                        )}`

                                        self.$store.dispatch(
                                            'builder_blockUpdateContent',
                                            {
                                                id: self.block.id,
                                                content: {
                                                    text: `${element.prop(
                        ...
                        renderLabel({ options, node }) {
                            return `${options.suggestion.char}${
                                node.attrs.label || node.attrs.id + '-Unnamed'
                            }`
                        },
                    }),
                    ParagraphList,
        ...
        parseBlockContent(block) {
            ...
            if (block.content.text !== null) {
                const matches = [
                    ...block.content.text.matchAll(
                        /{(\d+)}|{(\d+:(counter|value|number|currency|words|\d+))}|{(\d+:\d+:(words|currency|number))}/gm
                    ),
                ]
                matches.forEach((match) => {
                    const id = match[1] || match[2] || match[4]
                    const variable = this.variableUpdated.find(
                        (x) => x.id.toString() === id.toString()
                    )
                    if (variable) {
                        text = text.replace(
                            `{${id}}`,
                            `<span class="mention variable" data-type="mention" data-id='${id}' data-label='${variable.name}' contenteditable="false">@${variable.name}</span>`
                        )
                    }
                })
            }
