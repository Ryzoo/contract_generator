<template>
    <div :class="block.id > 1 ? '' : 'ignore-elements'" :data-id="block.id">
        <div
            class="block-header accordion-body"
        ...
        >
            ...
            <div class="block-header--content" @click="toggleBlock($event)">
                <div class="block-header--type">
                    {{ Mapper.getBlockName(block.blockType) }}
                </div>
                <h5 class="pr-2">
            ...
            <div class="block-header--action mr-4 text-right">
                ...
                <v-btn small text color="info" @click="copyBlock()">
                    {{ $t('base.button.copy') }}
                    <v-icon small right>fa-copy</v-icon>
                </v-btn>
                <v-btn small text color="primary" @click="editBlock()">
                    {{ $t('base.button.edit') }}
                    <v-icon small right>fa-edit</v-icon>
                </v-btn>
                <v-btn small text color="error" @click="deleteDialog = true">
...
export default {
    ...
    computed: {
        ...
        currentListEnumeratorType() {
            return this.$store.getters.builder_currentListEnumeratorType(
                this.block.id
            )
        },
        currentMultiGroupAttribute() {
            return this.$store.getters.builder_currentMultiGroupAttribute(
                this.block.id
            )
        },
    },
