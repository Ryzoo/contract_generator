    ...
    <v-row>
      <v-col>
        <h3>
          ...
          <span class="light-text" v-if="$store.getters.getNewContractData">{{
            $store.getters.getNewContractData.name
          }}</span>
        </h3>
      ...
      <v-col cols="auto">
        <v-row>
          <v-col>
            <v-btn small block color="primary" outlined @click="goBack">{{
              $t("base.button.configuration_back")
            }}</v-btn>
          </v-col>
          <v-col>
