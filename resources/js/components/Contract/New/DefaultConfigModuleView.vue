  ...
  <div class="config-module-view">
    ...
    <v-dialog v-model="configureDialog" width="800px">
      <v-card>
        <v-card-title>
          <span class="headline"
            >{{ $t("module.header.moduleConfiguration") }}
            {{ module.name }}</span
          >
        </v-card-title>
        <v-card-text>
          <component
            :ref="module.slug"
            :module="module"
            :is="module.configComponent"
          />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn outlined color="primary" @click="configureDialog = false">{{
            $t("base.button.close")
          }}</v-btn>
  ...
  methods: {
    ...
    checkRequired() {
      if (!this.isOn && this.module.required) {
        this.changeModuleState(true).then(() => {
