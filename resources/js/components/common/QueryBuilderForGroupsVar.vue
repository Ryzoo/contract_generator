...
export default {
    ...
    data() {
        return {
            availableAttributes: this.attributesList.filter(
                (x) => x.id !== this.attribute.id
            ),
            componentKey: 0,
            conditionalRules: [],
            blockOptions: ['SHOW_ON'],
            content: JSON.parse(
                this.attribute.conditionals && this.attribute.conditionals[0]
                    ? this.attribute.conditionals[0].content || '{}'
                    : '{}'
            ),
    ...
    watch: {
        attributesList(newValue) {
            this.availableAttributes = newValue.filter(
                (x) => x.id !== this.attribute.id
            )
        },
        attribute(newValue) {
            this.content = JSON.parse(
                newValue.conditionals && newValue.conditionals[0]
                    ? newValue.conditionals[0].content || '{}'
                    : '{}'
            )
            this.componentKey += 1
        },
    ...
    computed: {
        rulesFromAttributes() {
            return this.availableAttributes.map((x) => {
                const type = this.mapBackendTypeToBuilderType(x)
                const options = x.settings.items
                    ? x.settings.items.map((e) => ({ label: e, value: e }))
                    : null
    ...
    methods: {
        onConditionalChange(newValue) {
            const data =
                newValue.children && newValue.children.length > 0
                    ? [
                          {
                              conditionalType: parseInt(
                                  ConditionalEnum[this.conditionalType]
                              ),
