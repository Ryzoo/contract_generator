...
class ConditionalParser {
    ...
    validate(conditionalType, attribute) {
        if (
            ...
            (attribute.conditionals &&
                Array.isArray(attribute.conditionals) &&
                attribute.conditionals.length === 0)
        ) {
            return true
        }
        ...
        if (Array.isArray(attribute.conditionals[0])) {
            return (
                attribute.conditionals.every((x) => !x.length) ||
                attribute.conditionals
                    .filter((x) => x.length)
                    .some((x) =>
                        x.every(
                            (c) =>
                                parseInt(c.conditionalType) !==
                                    parseInt(conditionalType) ||
                                this.isConditionalValidAndEqual(
                                    ModelObjectToTextParser.parse(
        ...
        return attribute.conditionals.every(
            (c) =>
                parseInt(c.conditionalType) !== parseInt(conditionalType) ||
                this.isConditionalValidAndEqual(
                    ModelObjectToTextParser.parse(JSON.parse(c.content)),
                    true
                )
        )
    }
    ...
    parseConditionalStringToBool(content) {
        const contentWithVariables = content
            .split(' ')
            .map((e) =>
                e.replace(/{(\d+)}/g, (m, id) => this.getVariableValue(id))
            )
            .map((e) =>
                e.replace(/{(\d+:\d+)}/g, (m, id) => this.getVariableValue(id))
            )
            .map((e) =>
                e.replace(/{(\d+:value)}/g, (m, id) =>
                    this.getVariableValue(id)
                )
            )
            .map((e) =>
                e.replace(/{(\d+:counter)}/g, (m, id) =>
                    this.getVariableValue(id)
                )
            )
    ...
    getVariableValue(varId) {
        ...
        if (varId.includes(':')) {
            ...
            if (attr) {
                ...
                if (
                    !attr.settings.isMultiUse &&
                    attr.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP
                ) {
                    foundedAttribute = attrVal.find(
                        (y) => parseInt(y.id) === parseInt(value)
                    )
                } else if (
                    attr.settings.isMultiUse &&
                    attr.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP
                ) {
                    foundedAttribute = attrVal[0].find(
                        (y) => parseInt(y.id) === parseInt(value)
                    )
            ...
            foundedAttribute = allAttributes.find(
                (e) => parseInt(e.id) === parseInt(varId)
            )
        }
        ...
        if (
            foundedAttribute.value instanceof Object &&
            'bool' in foundedAttribute.value
        ) {
            if (!foundedAttribute.value.bool) return null

            if (
                typeof foundedAttribute.value.input === 'string' &&
                isNaN(parseFloat(foundedAttribute.value.input))
            ) {
        ...
        return !!foundedAttribute.value || foundedAttribute.value === 0
            ? foundedAttribute.value
            : null
    }
}
