export default class Auth {
  ...
  checkPermission(permission) {
    ...
    if (
      ...
      !this.store.getters.authUser.permissions
    ) {
      return false;
    }

    return this.store.getters.authUser.permissions.some(
      (x) => x.slug === permission
    );
  }
  ...
  checkRole(role) {
    ...
    if (!this.store.getters.authUser || !this.store.getters.authUser.roles) {
      return false;
    }
  ...
  checkAuth() {
    return new Promise((resolve, reject) => {
      ...
      let loginTokenStore =
        this.store && this.store.getters.authUser
          ? this.store.getters.authUser.loginToken
          : "";
      ...
      if (loginTokenStore) {
        window.axios.defaults.headers.common.Authorization =
          "Bearer " + loginTokenStore;
        axios
          .post("/auth/authorize", { loginToken: loginTokenStore })
          .then((response) => {
            if (response.data) {
