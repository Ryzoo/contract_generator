...
const mutations = {
    ...
    NEW_CONTRACT_UPDATE_MODULE_STATE: (state, data) => {
        if(!data.value)
            state.contract.settings.enabledModules = state.contract.settings.enabledModules.filter(x=>x !== data.name);
        else{
            state.contract.settings.enabledModules.push(data.name);

            if(!state.contract.settings.modules[data.name])
                state.contract.settings.modules[data.name] = data.settings;
        }
    },
