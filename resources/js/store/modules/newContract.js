...
const mutations = {
  ...
  NEW_CONTRACT_UPDATE_MODULE_STATE: (state, data) => {
    if (!data.value) {
      state.contract.settings.enabledModules =
        state.contract.settings.enabledModules.filter((x) => x !== data.slug);
    } else {
...
const getters = {
  ...
  getModuleSettings: (state) => (moduleName) =>
    state.contract.settings.modules[moduleName],
  newContract_availableModules: (state) =>
    state.contract.settings.enabledModules,
};
