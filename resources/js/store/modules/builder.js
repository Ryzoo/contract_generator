...
const mutations = {
    BUILDER_COPY_BLOCK: (state, block) => {
        ...
        const newBlockData = {
            ...
            content: copyContentOfBlock(
                state,
                block.content,
                state.builder.idBlockIncrement
            ),
        }
        state.builder.blocks = copyBlockTo(
    ...
    BUILDER_DRAG_BLOCKS: (state, data) => {
        ...
        state.builder.blocks = removeFromData(state.builder.blocks, blockId)
        state.builder.blocks = addNewBlockToCurrentBlocks(
            state.builder.blocks,
            copiedBlock,
            parseInt(placeIndex) === 0 && parseInt(destinationBlock) === 0
                ? 1
                : parseInt(placeIndex)
        )
    },
        ...
        attributes.forEach((attribute) => {
            if (attribute.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP) {
                attribute.settings.attributes =
                    attribute.settings.attributes.map((attributeIn) => {
    ...
    VARIABLES_UPDATE_CONDITIONALS: (state, data) => {
        state.builder.variables = state.builder.variables.map((attribute) => {
            if (parseInt(attribute.id) === parseInt(data.id)) {
                attribute.conditionals = [...data.value]
            }
    ...
    BUILDER_UPDATE_GROUP_VARIABLE: (state) => {
        ...
        state.builder.variables = state.builder.variables.map((attribute) => {
            if (
                parseInt(attribute.attributeType) ===
                AttributeTypeEnum.ATTRIBUTE_GROUP
            ) {
                attribute.settings.attributes =
                    attribute.settings.attributes.map((x) => {
    ...
    BUILDER_UPDATE_CURRENT_MULTI_ATTRIBUTE: (state, data) => {
        const updateBlock = (blocks, dataValue) => {
            return blocks.map((x) => {
                if (parseInt(x.id) === parseInt(dataValue.id)) {
                    x.content.blocks = [
                        ...removeVarIdFromBlocks(
                            x.content.blocks,
                            x.settings.repeatAttributeId
                        ),
                    ]
                    x.settings = {
                        ...x.settings,
    ...
    BUILDER_REMOVE_VARIABLE: (state, Id) => {
        state.builder.variables = state.builder.variables.filter(
            (item) => item.id !== Id
        )
...
const getters = {
    ...
    builder_allVariables_queryBuilder_block: (state) => (blockId) => {
        ...
        if (
            block.settings.repeatAttributeId !== null &&
            block.settings.repeatAttributeId !== undefined
        ) {
        ...
        if (state.builder.activeBlockNestedAttributes) {
            state.builder.activeBlockNestedAttributes.forEach((x) => {
                if (!attributes.some((y) => String(y.id) === String(x.id))) {
                    attributes.push(x)
                }
            })
        }
    ...
    builder_allVariables_defaultText: (state) => {
        ...
        state.builder.variables
            .filter(
                (x) => x.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP
            )
            .forEach((attribute) => {
                attribute.settings.attributes.forEach((x) => {
        ...
        const varForCount = state.builder.variables.filter(
            (x) =>
                x.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP &&
                !!x.settings.isMultiUse &&
                !x.settings.isInline
        )
        ...
        const allVar = state.builder.variables
            .filter(
                (x) =>
                    !x.settings.isMultiUse ||
                    (!!x.settings.isMultiUse && !!x.settings.isInline)
            )
        ...
        allVar.forEach((attribute) => {
            if (attribute.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP) {
                attribute.settings.attributes.forEach((x) => {
                    if (
                        parseInt(x.attributeType) === AttributeTypeEnum.CURRENCY
                    ) {
                        returnedVar.push({
                            ...x,
                            attributeName:
    ...
    builder_multiGroupAttributes: (state) =>
        state.builder.variables.filter(
            (x) => !!x.settings.isMultiUse && !x.settings.isInline
        ),
    builder_variablesForRepeatBlock: (state) => (id, nestedAttributes) => {
        ...
        const attribute = getAttributeById(
            state.builder.variables,
            block ? block.settings.repeatAttributeId : null
        )
        ...
        if (attribute) {
            if (attribute.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP) {
                const repeatAttribute = attribute.settings.attributes.map(
                    (x) => {
                        allAttributes = allAttributes.filter(
                            (y) => y.id !== parseInt(x.id)
                        )
                        return {
                            ...x,
                            attributeName:
                ...
                attrToReturn = [...repeatAttribute, ...allAttributes]
            } else {
                attrToReturn = [
                    ...allAttributes,
                    {
...
const getAttributeById = (attributes, id) =>
    attributes.find((x) => parseInt(x.id) === parseInt(id))
    ...
    if (parseInt(newBlock.parentId) === 0) {
        ...
        blocks = blocks.map((x) => {
            if (parseInt(x.id) === parseInt(newBlock.parentId)) {
                ...
                typeof x.content.blocks !== 'undefined' &&
                x.content.blocks.length > 0
            ) {
                x.content.blocks = addNewBlockToCurrentBlocks(
                    x.content.blocks,
                    newBlock,
                    index
                )
...
const removeVarIdFromBlocks = (blocks, varId) => {
    const regex = new RegExp(
        `\\{${varId}\\}|\\{${varId}:counter\\}|\\{${varId}:\\d+\\}|\\{${varId}:value\\}|${varId}|${varId}:counter|${varId}:\\d+|${varId}:value`,
        'gm'
    )
    return blocks.map((x) => {
        if (x.conditionals) {
            x.conditionals = x.conditionals.filter(
                (c) => !regex.test(c.content)
            )
        }
