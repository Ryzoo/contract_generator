...
const mutations = {
  ...
  BUILDER_ATTRIBUTE_IMPORT: (state, attributes) => {
    attributes.forEach((attribute) => {
      if (attribute.attributeType === AttributeTypeEnum.ATTRIBUTE_GROUP) {
        attribute.settings.attributes = attribute.settings.attributes.map(
          (attributeIn) => {
  ...
  BUILDER_UPDATE_GROUP_VARIABLE: (state) => {
    ...
    state.builder.variables = state.builder.variables.map((attribute) => {
      if (
        parseInt(attribute.attributeType) === AttributeTypeEnum.ATTRIBUTE_GROUP
      ) {
        attribute.settings.attributes = attribute.settings.attributes.map(
          (x) => {
        ...
        attribute.settings.attributes.forEach((x) => {
          if (parseInt(x.attributeType) === AttributeTypeEnum.CURRENCY) {
            returnedVar.push({
              ...x,
              attributeName:
                attribute.attributeName + " - " + x.attributeName + " - number",
            ...
            returnedVar.push({
              ...x,
              attributeName:
                attribute.attributeName + " - " + x.attributeName + " - words",
