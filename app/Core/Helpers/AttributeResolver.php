            ...
            foreach ($attributeIdList[1] as $id) {
                ...
                if (isset($attribute) && !((bool)$attribute->settings['isMultiUse'])) {
                    foreach ($attribute->value as $attribute) {
                        ...
                        $raw = $attributeParse->getRavValue();
                        $text = str_replace([
                            '{' . $id . ':' . $attributeParse->id . '}',
                            '{' . $id . ':' . $attributeParse->id . ':words}',
                            '{' . $id . ':' . $attributeParse->id . ':number}',
                            '{' . $id . ':' . $attributeParse->id . ':currency}',
                        ], [
                            $this->escapeValue($attributeParse->getValue()),
                            $this->escapeValue(method_exists($attributeParse, 'getWords') ? $attributeParse->getWords() : null),
                            $this->escapeValue($raw['number'] ?? null ),
                            $this->escapeValue($raw['currency'] ?? null)
                        ], $text);
                    }
                }
            }
